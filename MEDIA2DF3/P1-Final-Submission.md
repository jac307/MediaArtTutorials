[← MEDIA 2DF3](README.md) | [Project 1](P1-README.md)

---

<h1 style="color: darkred;">P1 – Final Submission: Assembly, Exhibition & Documentation</h1>  
*Group Project – 4 students per group*

---

The final submission is divided into **two parts**:

1. **In-Person Submission – Assembly & Class Exhibition**
2. **Online Submission – Final Project Documentation on Avenue to Learn**

Your final grade will consider the complete development of the project, including:

- Application of concepts from **Composition Techniques I and II**
- Craftsmanship and colour execution
- Consistency across the painted components
- Laser-cutting and engraving quality
- Structural assembly
- Exhibition presentation
- Quality and consistency of the final documentation

---

<!-- 
/////////////////
PART 1
/////////////////
-->

<details class="tutorial-section">
  <summary>
    <span class="section-title">Part 1. Assemble and bring the sculpture for the Class Exhibition</span>
    <span class="section-description">
      Complete the painting and assembly of the sculpture and bring the finished work to the final class exhibition.
    </span>
  </summary>

<div class="section-content" markdown="1">

## Complete the painting

Before assembling the sculpture, complete the painting of all planes, the base, and any additional structural components.

Follow the **approved colour palette** from Part 2.

### Painting requirements

- Use the approved **hand-mixed colours**.
- Apply a **white base coat** before the final colours where required.
- Apply **2–3 thin, even coats**.
- Allow each coat to dry completely.
- Keep brush strokes smooth and consistent.
- Paint all visible surfaces and edges.
- Maintain the planned colour relationships and progression.
- Preserve the visibility of intentional engraved textures.

> Visible brush strokes, inconsistent coverage, unintended colour changes, or unfinished surfaces may result in **grade deductions**.

## Assemble the sculpture

Once all components are fully painted and dry:

- Assemble the sculpture according to the approved design.
- Maintain the planned spacing, layering, direction, and structural relationships.
- Use **minimal glue**.
- Make sure all connections are secure.
- Keep the base and structural components clean.
- Remove or avoid visible glue marks.
- Check the sculpture from multiple perspectives before considering it complete.

> The final assembly should reflect the structural and compositional decisions developed during Parts 1 and 2.

## Class Exhibition

Your group must bring the **fully completed sculpture to the final class exhibition**.

The sculpture must be:

- Fully painted
- Fully assembled
- Completely dry
- Structurally stable
- Ready to exhibit when brought to class

The instructor will assign the location for displaying Project 1 during the exhibition.

> **Bringing the completed sculpture to the final class exhibition is a required part of the project. Failure to bring the work to the exhibition, or bringing an incomplete project, will result in a grade deduction.**

During the exhibition, the instructor will evaluate the physical work, including:

- Craftsmanship
- Colour application
- Colour relationships and proportioning
- Engraved texture
- Structural organization
- Assembly quality
- Stability
- Overall presentation

</div>
</details>

<!-- 
/////////////////
PART 2
/////////////////
-->

<details class="tutorial-section">
  <summary>
    <span class="section-title">Part 2. Complete and submit the final Project 1 documentation</span>
    <span class="section-description">
      Update the InDesign document with the final palette, professional photographs, and documentation of the sculpture in the class exhibition.
    </span>
  </summary>

<div class="section-content" markdown="1">

After the Class Exhibition, reopen the **Adobe InDesign document** developed throughout Parts 1 and 2.

You must create a **final version of the complete Project 1 document** that records the development and final presentation of the work.

> Maintain the same visual system established throughout the document. Pay attention to typography, hierarchy, margins, alignment, spacing, image placement, and consistency across pages.

## 1. Add the final scanned colour palette

Under the existing **Colour** section, add the **final approved version of the scanned physical colour palette**.

If your group revised the palette after instructor feedback, include the **final revised palette**, not the earlier version.

The scan should be clearly presented and large enough to evaluate the painted colours.

## 2. Add two professional photographs of the final sculpture

Include **two professional photographs** of the completed sculpture from **two distinctive perspectives**.

For example:

- Front and side
- Front and three-quarter view
- Low and elevated perspective
- Two views that clearly reveal different spatial relationships within the sculpture

The photographs should document the sculpture itself rather than the exhibition environment.

### Photography requirements

Consider consistency across both photographs:

|  |  |  |
|---|---|---|
| Lighting | Background | Framing |
| Camera angle | Image quality | Colour consistency |
| Focus | Scale | Visual presentation |

The photographs should:

- Be sharp and properly exposed.
- Use consistent lighting and colour.
- Avoid distracting backgrounds.
- Present the sculpture clearly.
- Show the form, colour, engraved texture, depth, and assembly.
- Be edited consistently if image adjustments are necessary.

> The two photographs should function as **professional documentation of the completed artwork**.

## 3. Add an Exhibition Documentation page

Create a separate page documenting the sculpture as it appeared in the **Class Exhibition**.

The page must include:

- **Name of the exhibition**
- **Date**
- **Location**
- **3–4 photographs of the sculpture in the exhibition context**

The exhibition photographs should show the relationship between the sculpture and its display environment.

Possible images may include:

- Full view of the installed sculpture
- Wider view showing the exhibition space
- Different perspective of the work in context
- Installation or display detail

## Image size and layout

All photographs must be large enough to evaluate clearly.

- The **largest images should occupy approximately 1/3 of a page**.
- The **smallest images should occupy approximately 1/4 of a page**.
- Do not use very small thumbnail images.
- Maintain consistent spacing and alignment.
- Keep image proportions intact.
- Do not stretch or distort photographs.
- Design each page so the photographs and written information form a **visually cohesive composition**.

> Consider the final PDF as a piece of **professional project documentation**, not simply a collection of images.

## Final document check

<fieldset class="equipment-checklist">
  <legend>Final Documentation Check</legend>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The complete Project 1 process is documented.</strong>
      The PDF includes the planning, development, colour approach, and final outcome.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The final colour palette is included.</strong>
      The approved scanned palette appears under the Colour section.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>Two professional photographs are included.</strong>
      The completed sculpture is documented from two distinctive perspectives.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The photographs are visually consistent.</strong>
      Lighting, colour, framing, image quality, and presentation are cohesive.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The Exhibition Documentation page is complete.</strong>
      It includes the exhibition name, date, location, and 3–4 photographs of the sculpture in context.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The photographs are appropriately sized.</strong>
      Images occupy approximately 1/3 of a page at the largest and 1/4 of a page at the smallest.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The document design is consistent.</strong>
      Typography, hierarchy, margins, alignment, spacing, and image placement remain cohesive throughout the PDF.
    </span>
  </label>

  <label class="checklist-item">
    <input type="checkbox">
    <span>
      <strong>The PDF has been reviewed before submission.</strong>
      All images and text display correctly and there are no missing or incorrectly positioned elements.
    </span>
  </label>

</fieldset>

</div>
</details>

---

## Final Submission

### Part 1 – In Person

Bring the **fully painted and assembled sculpture** to the final Class Exhibition.

> **Failure to bring the completed sculpture to the Class Exhibition will result in a grade deduction.**

### Part 2 – Avenue to Learn

Submit the **final updated Project 1 documentation PDF**:

| Item | Required filename |
|---|---|
| Final Project 1 Documentation | `Group-#-Project1-Final.pdf` |

The final PDF must include:

1. Original concept and design development
2. Material and structural planning
3. Colour planning
4. **Final scanned colour palette**
5. **Two professional photographs of the completed sculpture from distinctive perspectives**
6. **One Exhibition Documentation page with the exhibition name, date, location, and 3–4 photographs of the sculpture in context**

> **Failure to follow the documentation requirements, image specifications, exhibition requirements, or naming conventions may result in grade deductions.**

---

Credits: Jessica A. Rodríguez